...
def prepare_and_align(atoms, stack_axis):
    ...
    if src_idx != 2:
        ...
        new_lengths[2], new_lengths[src_idx] = (
            new_lengths[src_idx],
            new_lengths[2],
        )

    new_atoms = Atoms(
        symbols=atoms.get_chemical_symbols(), positions=new_pos, pbc=True
    )
    new_atoms.set_cell(
        [new_lengths[0], new_lengths[1], new_lengths[2], 90, 90, 90]
    )
...
# =====================================================================
# THAY ĐỔI CÁC THÔNG SỐ ĐẦU VÀO Ở ĐÂY
# =====================================================================

# Đường dẫn đến file vasp đầu vào (Giữ nguyên chữ r ở trước dấu ngoặc kép)
file1 = r"C:\Users\huyuy\Downloads\temp\LC_LIC.vasp"
axis1 = "z"  # Trục ghép của file 1 ('x', 'y', hoặc 'z')

file2 = r"C:\Users\huyuy\Downloads\temp\LIC_LC.vasp"
axis2 = "z"  # Trục ghép của file 2 ('x', 'y', hoặc 'z')

# Các thông số khoảng cách
gap_distance = 2.0  # Khoảng cách giữa 2 slabs (đơn vị Angstrom)
vacuum_distance = 0.0  # Khoảng cách chân không thêm vào ở 2 biên độ cao cell

# Thực thi hàm ghép file
merge_pro(
    f1=file1,
    ax1=axis1,
    f2=file2,
