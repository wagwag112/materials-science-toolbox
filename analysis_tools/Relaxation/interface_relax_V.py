...
def main():
    ...
    # 5. Best-energy tracking (positions + cell, since volume is also relaxed)
    ...
    print("  -> Phase 1: Interface relaxation (fmax=0.05, steps=50)...")
    ucf = UnitCellFilter(final_struct, mask=[1, 1, 0, 0, 0, 0])
    dyn = LBFGS(ucf, logfile="relax_interface.log")
    ...
    ucf_final = UnitCellFilter(final_struct, mask=[1, 1, 0, 0, 0, 0])
    dyn_final = LBFGS(ucf_final, logfile="relax_full_slab.log")
    dyn_final.attach(log_best_step, interval=1)

    try:
        dyn_final.run(fmax=0.05, steps=100)
    except Exception as e:
        print(f"  -> Error: {e}")

    # 8. Rollback to best structure (positions + cell)
    if best_positions is not None:
        ...
        if best_energy < final_e:
            ...
            final_struct.set_cell(best_cell)
        else:
