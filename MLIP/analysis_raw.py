...
def main():
    ...
    for T in temps:
        ...
        # Block Averaging Strategy
        block_size = len(frames) // N_BLOCKS
        block_diffusivities = []
        
        for b in range(N_BLOCKS):
            block_frames = frames[b*block_size : (b+1)*block_size]
            b_msd = compute_msd(block_frames, mobile_indices)
            try:
                # Use total end_fit_ps but limit it by block duration if necessary
                duration = (len(block_frames) * TIMESTEP_FS * LOG_INTERVAL) / 1000.0
                limit_ps = min(end_fit_ps, duration * 0.9)
                D, _, _, _ = fit_diffusivity(b_msd, start_fit_ps, limit_ps, 
